...
| `wan22_dual_gpu.py` | The ai-toolkit Wan 2.2 mixin (`Wan22DualGPUMixin`). Drop into `<ai-toolkit>/extensions_built_in/diffusion_models/wan22/wan22_dual_gpu.py`; then run [`patches/aitk/patch_aitk_wan_integration.py`](patches/aitk/patch_aitk_wan_integration.py) to wire it into `Wan225bModel` MRO + the `load_wan_transformer` flow. Env-gated by `WAN_DUAL_GPU=true`. |
...
For **Wan 2.2 video LoRA training in ai-toolkit** (the same trainer this repo's flagship FLUX.2 patch lives in): the drop-in mixin is at [`wan22_dual_gpu.py`](wan22_dual_gpu.py) (repo root); the two-edit integration patcher is at [`patches/aitk/patch_aitk_wan_integration.py`](patches/aitk/patch_aitk_wan_integration.py). Env-gated by `WAN_DUAL_GPU=true` (mirrors `FLUX2_DUAL_GPU`). The mixin mirrors `Flux2DualGPUMixin` exactly in shape — `setup_dual_gpu_distribution` places `rope` / `patch_embedding` / `condition_embedder` / `blocks[0:split_at]` on cuda:0; `blocks[split_at:]` / `norm_out` / `proj_out` / `scale_shift_table` on cuda:1. Wan-specific wrinkles handled: `rotary_emb` is a `(cos, sin)` tuple, `scale_shift_table` is an `nn.Parameter` (not a module), Wan 2.2 ti2v's 2D-timestep path is preserved in the split forward. **Validated end-to-end on 2× RTX 5090, 2026-05-13** — Wan 2.2 TI2V-5B trained 50 steps at 1.86 it/s sustained on a sumi v9 image-only dataset (`do_i2v: false` for image-only training), checkpoint saved, loss trajectory clean. The 14B A14B variants (T2V-A14B, I2V-A14B) are wired the same day in `wan22_14b_model.py` — the integration patcher now adds `Wan22DualGPUMixin` to `Wan2214bModel`'s MRO (`Wan2214bI2VModel` inherits it for free) and calls `setup_dual_gpu_distribution` separately on each of the two expert transformers (`transformer_1` high-noise + `transformer_2` low-noise) after each is quantized. The `DualWanTransformer3DModel` wrapper's timestep-routed forward is unchanged — whichever expert it picks now runs through our split forward. **A14B validated end-to-end on 2× RTX 5090, 2026-05-13** — Wan 2.2 T2V-A14B trained 50 steps at 1.09 s/it sustained with `switch_boundary_every: 10` (so both high-noise and low-noise experts were exercised), both experts distributed (`Distributing Transformer 1/2 across cuda:0/cuda:1` in the load log), checkpoint saved, loss clean. I2V-A14B inherits `Wan2214bModel` so it's code-complete on the same path.
